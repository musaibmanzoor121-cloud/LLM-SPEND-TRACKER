...
import { KeyRound, Plus, Trash2, ShieldCheck, Lock, CheckCircle2 } from 'lucide-react';
...
export default function Keys() {
  ...
  return (
    <div className="flex flex-col space-y-8 animate-in fade-in duration-300">
      ...
      {/* Add New Key Form (3D Tactile Card) */}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <input 
                  ...
                  className="w-full bg-[#ECEFF4] border border-slate-300/80 rounded-xl px-3.5 py-2.5 text-slate-900 focus:outline-none focus:border-indigo-500 text-xs font-medium transition-all shadow-[inset_0_1px_2px_rgba(0,0,0,0.03)]"
                />
              </div>
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-slate-700">Secret API Key</label>
              <input 
                type="password" 
                value={keyValue}
                ...
                required
                placeholder="sk-..."
                className="w-full bg-[#ECEFF4] border border-slate-300/80 rounded-xl px-3.5 py-2.5 text-slate-900 font-mono focus:outline-none focus:border-indigo-500 text-xs transition-all shadow-[inset_0_1px_2px_rgba(0,0,0,0.03)]"
              />
            </div>
            <div className="pt-2">
              <button 
              ...
              >
                <ShieldCheck size={14} />
                <span>{submitting ? 'Encrypting & Saving...' : 'Save Encrypted Key'}</span>
              </button>
            </div>
          </form>
        </div>
      ...
      {/* Connected Keys Table (3D Tactile Card) */}
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-slate-200/70 text-xs">
                {loading ? (
                  ...
                  keys.map((key) => (
                    <tr key={key.id} className="hover:bg-[#ECEFF4]/60 transition-colors">
